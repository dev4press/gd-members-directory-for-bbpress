...
<ul id="bbp-member-<?php echo $member->ID; ?>" <?php gdmed_members_query()->member_class(); // phpcs:ignore WordPress.Security.EscapeOutput ?>>
    <li class="bbp-member-info">
		<?php do_action( 'bbp_theme_before_member_avatar', $member->ID ); ?>
		...
		<?php do_action( 'bbp_theme_before_member_name', $member->ID ); ?>

        <a class="bbp-member-name" href="<?php bbp_user_profile_url( $member->ID ); ?>">
			<?php echo esc_html( $member->display_name ); ?>
        </a>

		<?php do_action( 'bbp_theme_after_member_name', $member->ID ); ?>

        <p class="bbp-member-meta">
			<?php do_action( 'bbp_theme_before_member_meta', $member->ID ); ?>

			<?php echo $member->get_meta_info(); // phpcs:ignore WordPress.Security.EscapeOutput ?>

			<?php do_action( 'bbp_theme_after_member_meta', $member->ID ); ?>
        </p>
    </li>
    <li class="bbp-member-statistics">
		<?php do_action( 'bbp_theme_before_member_statistics', $member->ID ); ?>
		...
		<?php do_action( 'bbp_theme_after_member_statistics', $member->ID ); ?>
    </li>
    <li class="bbp-member-activity">
		<?php do_action( 'bbp_theme_before_member_activity', $member->ID ); ?>

        <p><?php echo $member->get_latest_activity(); // phpcs:ignore WordPress.Security.EscapeOutput ?></p>

		<?php do_action( 'bbp_theme_after_member_activity', $member->ID ); ?>
    </li>
</ul>
